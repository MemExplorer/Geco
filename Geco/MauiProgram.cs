...
public static class MauiProgram
{
	...
	static MauiAppBuilder InitializeGeminiServices(this MauiAppBuilder builder)
	{
		...
		{
			SystemInstructions =
			"You are Geco, a large language model based on Google Gemini. You are developed by SS Bois. Your response should always be sustainability focused. The 'FullContent' property should always be in HTML format. The biggest font size must not exceed h2 and do not include the main heading.",
			Conversational = false,
			ResponseMimeType = "application/json",
			ResponseSchema = new Schema(
			SchemaType.ARRAY,
