...
public class DatabaseTest
{
	...
	async Task PromptDatabaseTest()
	{
		...
		string searchUserPrompt = await promptRepo.BuildSearchUserBasedPrompt("What are trending fashion globally");
		_output.WriteLine($"Search User Prompt: {searchUserPrompt}");

		string searchCtgPrompt = await promptRepo.BuildSearchCtgBasedPrompt(SearchPredefinedTopic.Energy);
		_output.WriteLine($"Search Predefined Category Prompt: {searchCtgPrompt}");

		string triggerPrompt = await promptRepo.BuildTriggerNotifPrompt(DeviceInteractionTrigger.BrowserUsageUnsustainable);
		_output.WriteLine($"Trigger Prompt: {triggerPrompt}");

		string sustLikelihoodPrompt = await promptRepo.BuildSustLikelihoodPrompt("16.55%", "current_sustainability_likelihood = (7/10) * (12/20) * (10/16) * (29/46)", "Charging: Total frequency – 10, Frequency Sustainable Charging – 3, Frequency Unsustainable Charging – 7");
