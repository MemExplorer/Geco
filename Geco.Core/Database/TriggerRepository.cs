...
public class TriggerRepository : DbRepositoryBase
{
	...
	public async Task<IList<TriggerInfo>> FetchTriggerRecords()
	{
		...
		await using var fetchQuery = await db.ExecuteReader("SELECT Type, SUM(RawValue) FROM TblTriggerLog WHERE (unixepoch() - Timestamp) <= 604800 GROUP BY Type");
		while (fetchQuery.Read())
			triggers.Add(new((DeviceInteractionTrigger)(long)fetchQuery["Type"], (int)(long)fetchQuery["SUM(RawValue)"]));
